---
name: sk-hynix-stock-orchestrator
description: "SK하이닉스(000660) 주식 가치를 다관점에서 분석해 통합 보고서를 작성하는 에이전트 팀 오케스트레이터. 펀더멘털·기술적·산업·거시·시장심리 5개 영역을 병렬 분석한 뒤 통합·교차검증해 최종 보고서를 생성한다. 트리거: 'SK하이닉스 주식 분석', 'SK하이닉스 가치 평가', 'SK하이닉스 적정주가', 'SK하이닉스 투자 보고서', '000660 분석', 'equity research SK하이닉스'. 후속 작업: SK하이닉스 보고서 다시 실행, 부분 재실행, 업데이트, 수정, 보완, 이전 결과 개선, '펀더멘털만 다시', '기술적 부분만 갱신' 등 요청 시에도 반드시 이 스킬을 사용. 단순 시세 조회·뉴스 요약은 이 스킬의 범위가 아님(이때는 직접 응답)."
---

# SK Hynix Stock Orchestrator

SK하이닉스(000660)의 주식 가치를 5명의 전문 애널리스트 + 1명의 통합/QA 리뷰어로 구성된 에이전트 팀으로 분석하여, 사실·추정·의견이 명확히 구분된 최종 투자 보고서를 생성하는 오케스트레이터.
...
| industry | `industry-analyst` | 메모리·HBM·NAND·CIS 산업·경쟁 | `equity-research-method/references/semiconductor-industry.md` | `_workspace/02_industry_analysis.md` |
...
2. **사용자 요청 파싱**:
   ...
   - 출력 경로 (default: 작업 디렉토리의 `SK하이닉스_주식분석_보고서_{YYYYMMDD}.md`)
3. **작업 디렉토리 준비**:
...
```
TeamCreate(
  team_name: "sk-hynix-equity-team",
  members: [
    { name: "fundamental", agent_type: "fundamental-analyst", model: "opus",
      prompt: "SK하이닉스(000660) 펀더멘털·밸류에이션 분석. equity-research-method 스킬과 references/valuation.md 로드. 출력: _workspace/02_fundamental_analysis.md. 분석 기준일: {기준일}. 입력 노트: _workspace/00_input/request.md" },
    { name: "technical", agent_type: "technical-analyst", model: "opus",
      prompt: "SK하이닉스 기술적 분석. equity-research-method 스킬과 references/technical.md 로드. 출력: _workspace/02_technical_analysis.md. 분석 기준일: {기준일}." },
    { name: "industry", agent_type: "industry-analyst", model: "opus",
      prompt: "메모리·HBM·NAND·CIS 산업 분석. equity-research-method 스킬과 references/semiconductor-industry.md 로드. 출력: _workspace/02_industry_analysis.md. fundamental의 사업부문 매출이 들어오는 대로 활용." },
...
```
TaskCreate(tasks: [
  { title: "펀더멘털 분석", assignee: "fundamental", description: "재무 추세 + 사업부문 분해(DRAM/HBM/NAND/Solidigm/CIS) + 멀티플 + SOTP + 간이 DCF" },
  { title: "기술적 분석", assignee: "technical", description: "추세·MA·모멘텀·지지저항·3시나리오" },
  { title: "산업 분석", assignee: "industry", description: "DRAM 사이클 + HBM 선두 포지션 + NAND/Solidigm + CIS + 경쟁 매트릭스" },
  { title: "거시 분석", assignee: "macro", description: "환율·금리·CapEx·외인흐름·SOX·엔비디아 상관" },
...
3. reviewer에게 SendMessage:
   ...
   사용자 지정 경로 또는 default 경로(SK하이닉스_주식분석_보고서_{YYYYMMDD}.md)에 복사하라.
   ```
...
2. 팀 정리:
   ```
   TeamDelete(team_name: "sk-hynix-equity-team")
   ```
